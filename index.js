...
// Owner numbers
const ownerNumber = ["2349116370966"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("𝑷𝑹𝑶𝑺-𝑿𝑫~")) {
    console.log("❌ Invalid 𝑷𝑹𝑶𝑺-𝑿𝑫session format");
    process.exit(1);
  }
  ...
  console.log("♻️ 𝑷𝑹𝑶𝑺-𝑿𝑫session restored successfully");
}
...
async function connectToWA() {
  ...
  console.log(`🤖 𝑷𝑹𝑶𝑺-𝑿𝑫Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ 𝑷𝑹𝑶𝑺-𝑿𝑫Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '2349116370966@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
